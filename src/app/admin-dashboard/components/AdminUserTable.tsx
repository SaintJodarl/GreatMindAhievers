    ...
    >
      ...
      {selectedRows.size > 0 && (
        <div
          ...
          style={{ background: 'rgba(108,71,255,0.1)', borderBottom: '1px solid rgba(108,71,255,0.2)' }}
        >
      ...
      <div className="overflow-x-auto">
            ...
            {paged.map((user, i) => (
              <tr
                ...
                style={{
                  ...
                  background: selectedRows.has(user.id) ? 'rgba(108,71,255,0.06)' : i % 2 === 0 ? 'transparent' : 'rgba(255,255,255,0.01)',
                }}
              >
                ...
                <td className="px-4 py-3 text-xs font-mono-nums font-semibold" style={{ color: 'var(--primary)' }}>
                  {user.memberId}
                </td>
                ...
                <td className="px-4 py-3 text-xs font-mono-nums font-semibold" style={{ color: 'var(--accent)' }}>
                  ₦{user.walletBalance.toLocaleString()}
                </td>
                <td className="px-4 py-3">
                  <span
                    className={`badge ${
                      user.kycStatus === 'Approved' ?'badge-active'
                        : user.kycStatus === 'Under Review' ?'badge-pending'
                        : user.kycStatus === 'Rejected' ?'badge-rejected' :'badge-suspended'
                    }`}
                  >
                    {user.kycStatus}
                  </span>
                ...
                <td className="px-4 py-3">
                  <span
                    className={`badge ${
                      user.accountStatus === 'Active' ?'badge-active'
                        : user.accountStatus === 'Pending KYC' ?'badge-pending' :'badge-suspended'
                    }`}
                  >
                    {user.accountStatus}
                  </span>
                  ...
                  <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                    ...
                    >
                      <svg width="13" height="13" viewBox="0 0 13 13" fill="none">
                        <path d="M1 6.5s2.5-4.5 5.5-4.5 5.5 4.5 5.5 4.5-2.5 4.5-5.5 4.5S1 6.5 1 6.5z" stroke="currentColor" strokeWidth="1.3" />
        ...
        {paged.length === 0 && (
          <div className="py-16 text-center">
            <p className="text-sm font-medium mb-1" style={{ color: 'var(--foreground)' }}>No members found</p>
      ...
      <div className="flex items-center justify-between p-4 border-t" style={{ borderColor: 'var(--border)' }}>
        ...
        <div className="flex items-center gap-1">
          <button onClick={() => setPage((p) => Math.max(1, p - 1))} disabled={page === 1} className="px-2.5 py-1.5 rounded text-xs font-medium transition-colors disabled:opacity-40" style={{ background: 'var(--muted)', color: 'var(--foreground)' }}>←</button>
          {Array.from({ length: totalPages }, (_, i) => i + 1).map((p) => (
            <button key={`apage-${p}`} onClick={() => setPage(p)} className="w-7 h-7 rounded text-xs font-medium transition-all" style={page === p ? { background: 'var(--primary)', color: '#fff' } : { background: 'var(--muted)', color: 'var(--muted-foreground)' }}>{p}</button>
          ))}
          <button onClick={() => setPage((p) => Math.min(totalPages, p + 1))} disabled={page === totalPages} className="px-2.5 py-1.5 rounded text-xs font-medium transition-colors disabled:opacity-40" style={{ background: 'var(--muted)', color: 'var(--foreground)' }}>→</button>
        </div>
      </div>
...
function AdminActionModal({ modal, onClose }: { modal: NonNullable<ActionModal>; onClose: () => void }) {
          ...
          <>
            ...
            <div className="space-y-2">
              {[
                ...
                <div key={`detail-${k}`} className="flex justify-between text-xs py-1.5 border-b" style={{ borderColor: 'var(--border)' }}>
                  <span style={{ color: 'var(--muted-foreground)' }}>{k}</span>
                  <span className="font-medium font-mono-nums" style={{ color: 'var(--foreground)' }}>{v}</span>
                </div>
              ))}
            </div>
          ...
          <>
            <div className="flex items-center gap-3 mb-4">
              <div
                ...
                style={{
                  background: modal.type === 'suspend' ? 'rgba(255,77,106,0.1)' : 'rgba(16,217,160,0.1)',
                }}
              >
            ...
            <p className="text-sm mb-5" style={{ color: 'var(--secondary-foreground)' }}>
              {modal.type === 'suspend' ?'This will prevent the member from logging in, earning commissions, or requesting withdrawals. This action can be reversed.' :'This will restore full platform access for this member, including commissions and withdrawals.'}
            </p>
            {done ? (
              <div className="text-center py-2" style={{ color: 'var(--accent)' }}>
                <svg width="24" height="24" viewBox="0 0 24 24" fill="none" className="mx-auto mb-1">
                  <path d="M4 12l6 6 10-10" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" />
                </svg>
              ...
              <div className="flex gap-3">
                ...
                >
                  ...
                  ) : modal.type === 'suspend' ? 'Confirm Suspend' : 'Confirm Activate'}
                </button>
              </div>
            )}
          </>
