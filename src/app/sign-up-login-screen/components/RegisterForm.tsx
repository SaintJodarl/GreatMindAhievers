...
const STATES = ['Lagos', 'Abuja', 'Kano', 'Rivers', 'Oyo', 'Enugu', 'Edo', 'Delta', 'Anambra', 'Ogun', 'Kaduna', 'Cross River', 'Other'];
...
export default function RegisterForm({ onSwitchToLogin }: RegisterFormProps) {
  ...
  const handleNext = async () => {
    ...
    if (step === 1) fieldsToValidate = ['firstName', 'lastName', 'email', 'phone', 'state'];
    ...
    <div className="animate-slide-up">
      ...
      {step === 1 && (
        <div className="flex items-center gap-4 mb-6">
          ...
          <span className="text-xs font-medium uppercase tracking-wider" style={{ color: 'var(--muted-foreground)' }}>or create an account</span>
          <div className="flex-1 h-px" style={{ background: 'var(--border)' }}></div>
        </div>
      )}
        ...
        {step === 1 && (
          <div className="space-y-4 animate-fade-in">
            ...
            <div>
              ...
              {errors.state && (
                <p className="mt-1 text-xs" style={{ color: 'var(--negative)' }}>{errors.state.message}</p>
              )}
            </div>
          </div>
        )}
