...
export default function UserDashboardContent() {
  ...
  return (
    <div className="space-y-6 pb-12 animate-fade-in">
      ...
      <div className="relative rounded-2xl overflow-hidden bg-gradient-to-r from-blue-600 via-indigo-600 to-violet-700 text-white shadow-md p-5 lg:p-6">
        {/* Background decorative patterns */}
        <div className="absolute inset-0 opacity-10 bg-[radial-gradient(circle_at_top_right,rgba(255,255,255,0.4),transparent)] pointer-events-none" />
        ...
        <div className="relative flex flex-col md:flex-row md:items-center md:justify-between gap-5 z-10">
          <div>
            ...
            <p className="mt-1 text-blue-100 text-xs lg:text-sm font-medium">
              Welcome back, <span className="font-bold text-white">{userName}</span>! Here's what's happening today.
            </p>
            <div className="flex items-center gap-2.5 mt-3 text-[11px] font-semibold text-blue-100">
              <span className="bg-white/10 px-2.5 py-0.5 rounded-full backdrop-blur-sm font-mono text-white">
                ID: {summary.referralCode || `GMA-${summary.id.slice(-5).toUpperCase()}`}
              </span>
              <span className={`px-2.5 py-0.5 rounded-full backdrop-blur-sm flex items-center gap-1 ${
                summary.status === 'ACTIVE' ? 'bg-emerald-500/20 text-emerald-300' : 'bg-amber-500/20 text-amber-300'
              }`}>
                <span className={`w-1 h-1 rounded-full ${summary.status === 'ACTIVE' ? 'bg-emerald-400' : 'bg-amber-400'}`} />
                {summary.status}
              </span>
            </div>
          </div>
          ...
          <div className="flex items-center gap-3 flex-shrink-0 self-start md:self-auto">
            ...
            <Link
              ...
              className="relative p-2.5 bg-white/10 hover:bg-white/20 rounded-xl transition-all duration-200 border border-white/10 hover:scale-105 active:scale-95"
              title="View News & Announcements"
            >
              <Bell size={18} className="text-white" />
              {announcementsCount > 0 && (
                <span className="absolute -top-1 -right-1 flex h-4 w-4 items-center justify-center rounded-full bg-rose-500 text-[10px] font-bold text-white ring-2 ring-indigo-600 animate-pulse">
                  {announcementsCount}
                </span>
              )}
            </Link>
            ...
            <Link
              ...
              className="flex items-center gap-2 bg-white hover:bg-indigo-50 text-indigo-700 font-bold px-4 py-2.5 rounded-xl transition-all duration-200 shadow-md hover:scale-[1.02] active:scale-[0.98] text-xs"
            >
              <Plus size={16} />
              Register Member
            </Link>
      ...
      <div className="flex items-center justify-between flex-wrap gap-4 border-b border-gray-200 pb-2">
        <div className="flex gap-1.5 p-1 bg-gray-100 rounded-2xl overflow-x-auto scrollbar-none max-w-full">
          {tabs.map((tab) => (
            <button
              key={`user-tab-${tab.id}`}
      ...
      {activeTab === 'overview' && (
        <div className="space-y-6 animate-fade-in">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div className="lg:col-span-2 bg-white rounded-2xl border border-gray-150 p-5 shadow-sm flex flex-col justify-between">
              <div>
                  ...
                  <div className="space-y-3">
                    {recentTransactions.map((txn) => {
                      ...
                      return (
                        <div
                          key={txn.id}
                          className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 p-3.5 rounded-xl bg-gray-50/50 hover:bg-gray-100/70 border border-gray-100 transition-all duration-200 group"
                        >
              ...
              <div>
                ...
                {loadingExtra ? (
                  ...
                  <div className="flex flex-col items-center justify-center py-16 text-center">
                    ...
                    <p className="text-xs text-gray-400 mt-1 max-w-xs">Share your referral link to build your team!</p>
                  </div>
                ) : (
                  <div className="space-y-3">
                    {recentReferrals.map((user) => {
                      ...
                      return (
                        <div
                          key={user.id}
                          className="flex items-center justify-between p-3 rounded-xl bg-gray-50/50 border border-gray-100 hover:bg-gray-100/50 hover:border-gray-200 transition-all duration-200"
                        >
